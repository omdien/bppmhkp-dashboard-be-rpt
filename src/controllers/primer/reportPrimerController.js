...
export const export_rincian_report_primer = async (req, res) => {
    try {
        ...
        if (!tgl_awal || !tgl_akhir) {
            return res.status(400).json({
                status: "error",
                message: "Range tanggal (tgl_awal & tgl_akhir) harus diisi."
            });
        }

        // 1. Ambil data yang sudah diflatten dan difilter unik dari Service
        ...
        // 2. Definisi Header
        sheet.columns = [
            { header: "ID Checklist", key: "idchecklist", width: 15 },
            { header: "Tgl Izin", key: "tgl_izin", width: 12 },
            { header: "ID Izin", key: "id_izin", width: 12 },
            { header: "Jenis Izin", key: "jenis_izin", width: 15 },
            { header: "Kd Izin", key: "kd_izin", width: 12 },
        ...
        // 4. Masukkan Data
        sheet.addRows(dataFlattened);

        // 5. Styling Body (Border & Alignment)
        sheet.eachRow((row) => {
            row.eachCell((cell) => {
                cell.border = {
                    top: { style: 'thin' },
        ...
        // 7. Kirim Response File
        const fileName = `Export_Rincian_Primer_${tgl_awal}_${tgl_akhir}.xlsx`;
        ...
        console.error("CONTROLLER_EXPORT_ERROR:", error);
        res.status(500).json({
            status: "error",
